...
properties = np.vstack([properties_ids, mass, rmax, r200c, cnfw, rhosnfw]).T


properties_df = pd.DataFrame(
    data=properties, columns=["ID_DMO", "M200c", "Rmax", "R200c", "Cnfw", "Rhosnfw"]
)

merged_matching_df = pd.merge(
    mass_matching_df, properties_df, on=["ID_DMO"], how="inner"
)
...
mergertree_data = np.vstack(
    [mergertree_ids, formation_time, n_mergers, mass_peak, vpeak]
).T

mergertree_df = pd.DataFrame(
    data=mergertree_data,
...
np.testing.assert_allclose(
    dmo_merged_df.R200c, dmo_merged_df.Group_R_Crit200, rtol=1e-3
)
...
hydro_merged_df = pd.merge(
    dmo_merged_df, hydro_df, on=["ID_HYDRO"], how="inner", suffixes=("_dmo", "_hydro")
)
...
no_duplicates_df = pd.merge(
    no_duplicates_df, n_gals_total.to_frame("N_gals_total").reset_index()
)

...
def hod_stellar_mass_summary(m200c_train, m200c_test,
    ...
    for threshold in stellar_mass_thresholds:
        ...
        hod_tpcfs.append(compute_tpcf(dmo_pos_test[n_gals_test_hod > 0])[1])

    return halo_occs, hod_cms, hod_tpcfs

def hod_summary(m200c_train, m200c_test, n_gals_train, n_gals_test, dmo_pos_test):
        halo_occ = hod.HOD(m200c_train, n_gals_train)
        n_gals_test_hod = halo_occ.populate_centrals(m200c_test)

        cm = confusion_matrix(n_gals_test, n_gals_test_hod)
        cm = cm.astype("float") / cm.sum(axis=1)[:, np.newaxis]
        hod_tpcf = compute_tpcf(dmo_pos_test[n_gals_test_hod > 0])[1]
...
def model_stellar_mass_summary(stellar_mass_test, 
    ...
    for threshold in stellar_mass_thresholds:
        ...
        cm = cm.astype("float") / cm.sum(axis=1)[:, np.newaxis]
        cms.append(cm)
        pred_tpcf.append(compute_tpcf(dmo_pos_test[n_gals_test_pred > 0])[1])

    return cms, pred_tpcf 


def model_summary(n_gals_test, n_gals_pred, dmo_pos_test):
    
    cm = confusion_matrix(n_gals_test, n_gals_pred)
    cm = cm.astype("float") / cm.sum(axis=1)[:, np.newaxis]
    pred_tpcf = compute_tpcf(dmo_pos_test[n_gals_pred>0])[1]
    return cm, pred_tpcf

def hydro_stellar_mass_summary(hydro_pos_test, stellar_mass_test, stellar_mass_thresholds):
    hydro_tpcf = []
    for threshold in stellar_mass_thresholds:
        n_gals_test = stellar_mass_test > threshold 
        hydro_tpcf.append(compute_tpcf(hydro_pos_test[n_gals_test > 0])[1])
    r_c = compute_tpcf(hydro_pos_test[n_gals_test > 0])[0]

    return r_c, hydro_tpcf 

def hydro_summary(hydro_pos_test, n_gals_test):

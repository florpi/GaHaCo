...
def test_set_metric(model, X_test, y_test, metric, metric_params):
    ...
    return metric(y_test, y_pred, **params)

def permutation(model, X_test, y_test):
    baseline = test_set_metric(model, X_test, y_test)
    imp = []
    for col in X_test.columns:
        save = X_test[col].copy()
        X_test[col] = np.random.permutation(X_test[col])
        m = test_set_metric(model, X_test, y_test)
        X_test[col] = save
...
def dropcol(model, X_train, y_train, 
    ...
    baseline = test_set_metric(model_, X_test, y_test, metric, metric_params)
    imp = []
    for col in X_train.columns:
        ...
        imp.append(baseline - drop_column)
    imp = np.array(imp)
    return imp

...
class Catalog:
    ...
    def load_immidiate_features(self, group_feature_list: list, sub_feature_list: list):
        ...
        self.v200c = (
            np.sqrt(
                self.snapshot.const.G
                * self.Group_M_Crit200
                / self.Group_R_Crit200
                / 1000.0
            )
    ...
    def save_features(self, output_filename: str, features_to_save: list):
        """
        ...
        df = pd.DataFrame(data=feature_list, columns=features_to_save)

        if remove_grouppos:
            df["x"] = self.GroupPos[:, 0] / 1000.0  # To Mpc/h
            df["y"] = self.GroupPos[:, 1] / 1000.0
            df["z"] = self.GroupPos[:, 2] / 1000.0
...
class HaloCatalog(Catalog):
    def __init__(self):
        ...
        self.halo_mass_cut = (
            self.snapshot.cat["Group_M_Crit200"][:] * self.snapshot.header.hubble
            > self.halo_mass_thresh
        )
        # Save IDs of haloes
...
class GalaxyCatalog(Catalog):
    def __init__(self, snapnum=99):
        ...
        sub_properties = ["SubhaloMassType"]

        self.snapshot.group_catalog(group_properties + sub_properties)

        self.halo_mass_cut = (
            self.snapshot.cat["Group_M_Crit200"][:] * self.snapshot.header.hubble > 0.0
        )
        ...
        self.N_halos = (self.snapshot.cat["Group_M_Crit200"])[self.halo_mass_cut].shape[
            0
        ]
        ...
        self.Group_M_Crit200 = (
            self.snapshot.cat["Group_M_Crit200"][self.halo_mass_cut]
            * self.snapshot.header.hubble
        )

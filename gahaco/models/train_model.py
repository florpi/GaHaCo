...
def main(argv):
    ...
    if FLAGS.upload:
        ...
        opt = Optimizer(
            config, api_key="VNQSdbR1pw33EkuHbUsGUSZWr", project_name="general"
        )
    else:
    ...
    dropcol_importance, pm_importance, gini_importance,cms,hod_cms,hydro_tpcf,pred_tpcf,hod_tpcf=([] for i in range(8))
    ...
    for train_idx, test_idx in skf.split(features, labels):
        ...
        if config['feature_optimization']['measure_importance']:
            ...
            imp = feature_importance.permutation(trained_model, 
                        x_test, y_test, metric_value, metric, config['metric']['params'])
    ...
    if config['feature_optimization']['measure_importance']:
        visualize.plot_feature_importance(dropcol_importance,
                                    x_train.columns,
                                    title='Drop column',
                                    experiment=experiment)
        visualize.plot_feature_importance(pm_importance,
                                    x_train.columns,
                                    title='Permute column',
                                    experiment=experiment)
        visualize.plot_feature_importance(gini_importance,
                                    x_train.columns,
                                    title='Gini impurity',
                                    experiment=experiment)
